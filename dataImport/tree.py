...
#Set first two leaves as two lowest frequencies
firstLeaf = Leaf(symbol[0], 0)
secondLeaf = Leaf(symbol[1], 1)
nodeLeaf = [firstLeaf, secondLeaf]

#sum frequencies for first node to start pattern
...
#continue building the frequency sum and comparing to next largest
for x in range (3,len(frequency)):
...
#build the array for export to JSON
codeNumbers = []
for b in nodeLeaf:
	st = str(b.finalCode)
	for c in nodeNode:
